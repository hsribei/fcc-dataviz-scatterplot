...
function fetchAndExecutewithJson(url, jsonConsumer) {
    fetch(url).then(function (response) {
        response.json().then(jsonConsumer);
    });
}
function makeAScatterplotGraph(data) {
    var margin = { top: 40, right: 100, bottom: 70, left: 70 }, width = 800 - margin.left - margin.right, height = 600 - margin.top - margin.bottom;
    // Draw base SVG
    var svg = d3
        .select("div#d3")
        .append("svg")
    ...
    y.domain([d3.max(data, function (d) { return d.Place; }) + 2, d3.min(data, function (d) { return d.Place; })]);
    // Add the Y Axis
    ...
    marker
        .append("circle")
        .attr("class", function (d) { return "marker " + (d.Doping ? "yesDoping" : "noDoping"); })
        .attr("cx", function (d) { return x(secondsBehindFastest(d)); })
    ...
    marker
        ...
        .attr("x", function (d) { return x(secondsBehindFastest(d)) + 5 + 10; })
        .attr("y", function (d) { return y(d.Place) + 4; });
    // Add marker legends
    var legend = svg.append("g").attr("class", "legend");
    var legendYesDoping = legend.append("g");
    legendYesDoping
        .append("circle")
    ...
    var legendNoDoping = legend.append("g");
    legendNoDoping
        .append("circle")
